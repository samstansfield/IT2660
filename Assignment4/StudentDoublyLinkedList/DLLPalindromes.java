 ...
 */
import java.util.*;
public class DLLPalindromes
{
    int size;
    char[] charArray;
    Stack<Character> stack = new Stack<Character>();
    DLLQueue<Character> queue = new DLLQueue<Character>();
    /**
     * Preprocessing will be done before the Palindrome class is constructed.  
     */
    public DLLPalindromes(int size)
    {
    ...
    public boolean checkPalindrome(String inputString)
    {
        ...
        for(int i =0;i<size;i++)
        {
            ...
            this.queue.enqueue(temp);  // add it to the back of the queue.
        }
        
        for(int i=0;i<size;i++)
        {
            Character temp1 = this.queue.dequeue();  //  take a character from the front of the queue.
